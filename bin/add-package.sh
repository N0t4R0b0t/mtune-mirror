#!/usr/bin/env bash
# bin/add-package.sh <arch> <pkgname> [--source upstream|local|aur|git] [--url URL] [--ref REF]
#
...
# (branch or tag; omit to track the repo's default branch). All handled in
# bin/build.sh resolve_src. Used by CLI + UI.
set -euo pipefail
source "$(dirname "$(readlink -f "$0")")/lib/common.sh"

arch="${1:?usage: add-package.sh <arch> <pkgname> [--source ..] [--url ..] [--ref ..]}"
pkg="${2:?missing pkgname}"; shift 2
src="upstream"; url=""; ref=""
while [ $# -gt 0 ]; do
  case "$1" in
...
case "$src"  in upstream|local|aur|git) ;; *) die "invalid source: $src" ;; esac

...
`pkgmirror-web` is a single stdlib-only Go binary with the SPA embedded via
`//go:embed`. It is a thin orchestration/presentation layer: it reads config through
`dasel`, reads state files, queries `systemd`/`journald`, and shells out to the
`bin/*.sh` scripts — **bash remains the single source of truth** for build logic. It
runs as the `pkgmirror` user (which has passwordless sudo) and binds localhost behind
nginx.
